...
```text
你是Python环境配置专家，需要分析给定代码的依赖并生成配置文件。
请仔细阅读所有代码文件，完成以下任务：
1. 识别所有导入的第三方库，排除Python标准库
2. 确定每个库的稳定兼容版本，避免版本冲突
3. 生成requirements.txt文件，格式为"库名==版本号"
4. 编写环境配置说明文档，包含硬件要求、Python版本、安装步骤
优先选择论文发表时间段的主流稳定版本，确保与代码API兼容。
```
...
### 5.5 测试智能体
#### 5.5.1 测试系统提示词
测试智能体的系统提示词明确其角色是专业的软件测试工程师，擅长运行代码并发现运行过程中的问题。提示词要求智能体按照代码的说明文档运行生成的代码，监控代码的完整运行过程，记录所有的控制台输出、警告信息和错误信息。如果代码运行成功，需要收集所有的输出结果，包括评估指标、可视化结果、日志文件等内容，整理成结构化的测试报告。如果代码运行失败，需要详细记录错误信息，包括错误类型、错误位置、错误栈信息等，尽可能提供详细的错误上下文，帮助后续的错误修复智能体定位问题。提示词还要求智能体评估代码的运行效率，记录代码的运行时间、内存占用、GPU使用率等性能指标，为后续的优化提供参考。精简版提示词示例如下：
```text
你是专业的软件测试工程师，负责运行代码并生成测试报告。
请按照以下步骤执行：
1. 按照文档说明安装依赖并运行代码
2. 全程监控运行状态，记录所有控制台输出和日志
3. 运行成功则收集所有评估指标、输出结果和性能数据
4. 运行失败则记录完整的错误信息、错误栈和上下文
5. 生成结构化测试报告，包含运行状态、耗时、结果/错误详情
...
### 5.6 错误修复智能体
#### 5.6.1 错误修复系统提示词
错误修复智能体的系统提示词明确其角色是资深的Python调试专家，擅长快速定位并修复代码中的错误。提示词要求智能体根据测试智能体提供的错误信息和运行日志，结合生成的代码和论文分析报告，分析错误的根本原因，制定合理的修复方案。修复过程需要严格遵循论文中的算法描述，不能随意修改算法的核心逻辑，对于论文中没有明确说明的细节，可以根据领域常识进行合理调整，但需要在注释中说明修改的原因。提示词要求智能体优先修复语法错误、依赖缺失错误、参数配置错误等简单问题，对于逻辑错误，需要仔细对照论文中的算法描述，定位逻辑偏差的位置，进行针对性的修复。修复完成后需要说明修复的内容和原因，方便后续的验证和审查。精简版提示词示例如下：
```text
...
### 5.7 结果验证智能体
#### 5.7.1 结果验证系统提示词
结果验证智能体的系统提示词明确其角色是专业的学术研究评估专家，擅长对比实验结果，评估复现质量。提示词要求智能体从论文分析报告中提取论文中报告的所有实验结果和评价指标，整理成结构化的对比基准。然后对比测试智能体返回的运行结果与论文中的结果，计算各项指标的差异程度，评估复现结果的一致性。如果结果差异在学术可接受的范围内（通常小于5%），则判定为复现成功；如果差异较大，则分析差异产生的原因，是由于实现细节的差异、超参数的不同、随机种子的影响还是其他原因导致的，提出针对性的优化建议。提示词要求智能体生成详细的复现质量评估报告，包含各项指标的对比表、差异分析、质量评分和优化建议。精简版提示词示例如下：
```text
你是学术研究评估专家，负责评估论文代码复现的质量。
请完成以下任务：
1. 从论文分析报告中提取所有报告的实验结果和评价指标作为基准
2. 对比代码运行结果与论文基准，计算各项指标的相对差异
3. 差异小于5%判定为复现成功，否则分析差异原因
4. 生成评估报告，包含指标对比、差异分析、质量评分和优化建议
评估需客观公正，严格以论文报告结果为评判标准。
```
